...
We can now also use the replicated MI to study how the similated deviance is different from the observed value. We look at topic 1, the replicated values and compare it to the true value.
...
Based on this we can calculate the deviance of that topic, the standardized distance between the replicated values and the true value.  
...
With these tools we can search for systematic discrepancies in the data. In this example we study the grouping of the State of the Union Addresses by president and by year. We start by reading in this meta data and add it to the model state file. 

To compare the results of the presidential grouping we will also create a random grouping into 43 groups to be able to compare the deviance. 
...
We start out by calculate the grouped IMI and MI by using presidents and the random group as the grouping variable $g$. 
...
We use this grouping to calculate MI on based on simulated data. Once again this may take some time to calculate.
...
Based on these grouped MI calculations we can study how the deviance is distributed compared to using documents and the random group as the grouping variable.
...
As we can see, the deviance of grouping by president is much higher than a random grouping with equal number of groups. Indicating that
